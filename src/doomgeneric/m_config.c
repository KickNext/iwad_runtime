...
{
    char *tempdir = getenv("IWADR_TEMP");

    if (tempdir != NULL && tempdir[0] != '\0')
    {
        return M_StringDuplicate(tempdir);
    }
...
// Return the filename component of an IWAD path.
//
static const char *M_IWADBaseName(char *iwadpath)
{
    const char *slash;
    const char *backslash;
    const char *separator;

    slash = strrchr(iwadpath, '/');
    backslash = strrchr(iwadpath, '\\');
    separator = slash;
...
static char *M_SanitizeSaveScopeBaseName(const char *basename)
{
    char *result;
    size_t i;
    size_t length;
    ...
    if (result == NULL)
    {
        I_Error("Failed to allocate IWAD savegame scope.");
    }
...
static void M_IWADContentHash(char *iwadpath, sha1_digest_t digest)
{
    FILE *stream;
    byte buffer[8192];
    ...
    stream = fopen(iwadpath, "rb");

    if (stream == NULL)
    {
        I_Error("Failed to open IWAD file '%s' to calculate savegame scope.",
                iwadpath);
    }
    ...
    {
        fclose(stream);
        I_Error("Failed to read IWAD file '%s' to calculate savegame scope.",
                iwadpath);
    }
...
static char *M_SaveGameIWADScope(char *iwadpath)
{
    ...
    basename = M_IWADBaseName(iwadpath);
    safe_basename = M_SanitizeSaveScopeBaseName(basename);
    M_IWADContentHash(iwadpath, digest);
    ...
    digest_hex[12] = '\0';
    scope_size = strlen(safe_basename) + 1 + strlen(digest_hex) + 1;
    ...
    M_snprintf(scope, scope_size, "%s-%s", safe_basename, digest_hex);
    free(safe_basename);
...
char *M_GetSaveGameDir(char *iwadpath)
{
    char *savegamedir;
    char *savegamescope;
    char *topdir;
    ...
    {
        // Keep save slots separate per exact IWAD content:
        // <configdir>/savegame/<basename>-<sha1-prefix>/doomsav0.dsg

        topdir = M_StringJoin(configdir, DIR_SEPARATOR_S, "savegame", NULL);
        M_MakeDirectory(topdir);

        savegamescope = M_SaveGameIWADScope(iwadpath);
        savegamedir = M_StringJoin(topdir, DIR_SEPARATOR_S, savegamescope,
