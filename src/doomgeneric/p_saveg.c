...
char *P_TempSaveGameFile(void)
{
    ...
    const size_t required_size = strlen(savegamedir) + 16;

    if (filename == NULL || filename_size < required_size)
    {
        filename_size = required_size;
        filename = realloc(filename, filename_size);
        if (filename == NULL)
        {
            I_Error("Failed to allocate savegame filename.");
        }
    }
    M_snprintf(filename, filename_size, "%stemp.dsg", savegamedir);
...
char *P_SaveGameFile(int slot)
{
    ...
    if (filename == NULL || filename_size < strlen(savegamedir) + 32)
    {
