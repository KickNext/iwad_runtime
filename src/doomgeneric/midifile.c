...
static unsigned int iwadr_swap_be32(unsigned int value)
{
...
static unsigned short iwadr_swap_be16(unsigned short value)
{
...
static boolean ReadTrackHeader(midi_track_t *track, FILE *stream)
{
    ...
    track->data_len = iwadr_swap_be32(chunk_header.chunk_size);
...
static boolean ReadFileHeader(midi_file_t *file, FILE *stream)
{
    ...
    if (!CheckChunkHeader(&file->header.chunk_header, HEADER_CHUNK_ID)
     || iwadr_swap_be32(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
                        iwadr_swap_be32(file->header.chunk_header.chunk_size));
        return false;
    }

    format_type = iwadr_swap_be16(file->header.format_type);
    file->num_tracks = iwadr_swap_be16(file->header.num_tracks);
...
{
    short result = iwadr_swap_be16(file->header.time_division);
