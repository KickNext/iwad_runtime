...
void iwadr_on_save_game_written(void);
...
{ 
    ...
    requested_weapon = DG_ConsumeWeaponSlotRequest();

    if (gamestate == GS_LEVEL
     && requested_weapon >= 0
     && requested_weapon < arrlen(weapon_keys))
    {
        cmd->buttons |= BT_CHANGE;
        cmd->buttons |= requested_weapon << BT_WEAPONSHIFT;
    }
